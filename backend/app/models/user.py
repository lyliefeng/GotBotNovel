...
class User(Base):
    ...
    user_id = Column(String(100), primary_key=True, index=True, comment="用户唯一标识；历史数据可能包含旧版前缀")
    ...
    linuxdo_id = Column(String(100), nullable=False, unique=True, index=True, comment="兼容旧版数据库保留的外部标识，本地用户保存 user_id")
